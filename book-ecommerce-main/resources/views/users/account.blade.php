...
@if(session()->has('add'))
	<script>swal('Add Address Successfully','','success');</script>
@endif
...
<main>
	<div class='account'>
		<div class="a_left pt-4">
		   <ul>
		      <li><img src="{{session()->get('img')}}"><p>{{$user->name}}</p></li>
		...
		<div class="a_right ml-md-3 mt-md-4 p-4">
			...
			<form class="person_info" action="account/update/profile" method="post" enctype="multipart/form-data">
				...
				<div><label>Your Name</label> <input type="text" name="name" value="{{$user->name}}" ></div>
				<div><label>Email Address</label><input type="text" name="email" value="{{$user->email}}" disabled></div>
				<div><label>Mobile</label><input type="text" name="mobile" value="{{$user->mobile}}" ></div>
			...
			<form class="person_adrs" action="account/action/address" method="POST">
				...
				<div> <label>Delivery Address</label> <textarea name="address" required>{{$user->address}}</textarea> </div>
				<div> <label>Zip Code</label> <input type="text" name="zcode" value="{{$user->zcode}}" required> </div>
				@php
					$btn = '';
					if($user->zcode=="" && $user->address=="")
						$btn = 'ADD';
					else
						$btn = 'UPDATE';
				@endphp
				<input type="text" name="btn" value="{{$btn}}" hidden="hide">
				<div> <input type="submit" name="submit_btn" class="btn btn-outline-primary" value="{{$btn}}">
				</div>
			</form>
